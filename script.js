...
let getSVGPoints = (path) => {
  ...
  if(rawPath) {
      rawPath.forEach((val, i) => {
        if (i % 2) {
            arr.push({ x: rawPath[i*2-1], y: rawPath[i*2] }); // 修正座標抓取邏輯
        }
      });
  }
  return arr;
...
function drawStar() {
  ...
  prepareDraw(treeBottomMaskPath);
  ...
  starTl
    ...
    .to(".pContainer, .sparkle", {
      ...
      x: treeBottomPath[0] ? treeBottomPath[0].x : 0, // 安全檢查
      y: treeBottomPath[0] ? treeBottomPath[0].y : 0
    })
    .to(".pContainer, .sparkle", {
      duration: 2,
...
function createSnow() {
  ...
  for (let i = 0; i < numSnowflakes; i++) {
    ...
    gsap.set(snowflake, { x: gsap.utils.random(0, 800), y: gsap.utils.random(-100, -10) });
    gsap.to(snowflake, {
      y: 650,
...
if(treePathMaskPath && treePotMaskPath){
